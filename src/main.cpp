...
void setup_main_app_options( CLI::App& app )
{
    ...
    app.add_flag_callback(
        "--version",
        [] () {
            LOG_BOLD << grenedalf_version();
            throw CLI::Success{};
        },
        "Print the grenedalf version and exit."
    );
}
...
int main( int argc, char** argv )
{
    ...
    CLI::App app{ grenedalf_header() };
    app.name( "grenedalf" );
    app.footer( "\n" + grenedalf_title() );

    // We use a custom formatter for the help messages that looks nicer.
    app.formatter( std::make_shared<GrenedalfFormatter>() );

    // We don't like short options in grenedalf. Reset the help option.
    ...
    // Grenedalf expects exactly one subcommand.

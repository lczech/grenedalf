...
if [[ ${base_name} != grenedalf* ]]; then
    echo -e "\e[31mWrong repository. Expect grenedalf.\e[0m"
    exit
else
...
# Replace version line in grenedalf header file.
echo "Replace version in src/tools/version.hpp"
sed -i "s/    return \".*\"; \/\/ #GRENEDALF_VERSION#/    return \"${version}\"; \/\/ #GRENEDALF_VERSION#/g" src/tools/version.hpp
...
if [[ $cont == 1 ]]; then
    github_release_url="https://github.com/lczech/grenedalf/releases/new"
